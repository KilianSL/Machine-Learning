...
class CharRNNDataset():
    def __init__(self, txt_file_path='data/data.txt', chunk_size=50, transform=None):
        ...
        letters = set(words) #returns the list of unique words in the raw text
        ...
        txt = list(words)#convert string to list
...
def generate(model, prime_str='a', str_len=150, temperature=0.75):
    ...
    for i in range(x.shape[1]-1): #for each input character except the last
        out, h = model.forward(x[:, i], h) #feed that character into the network
...
startingtext = ""
while startingtext != "000":
    startingtext = input("Starting text: ") 
